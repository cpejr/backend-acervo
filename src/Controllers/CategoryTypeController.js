...
class CategoryController {
  async create(req, res) {
    try {
      const category = await CategoryTypeModel.create(req.body);
  ...
  async read(req, res) {
    try {
      const category = await CategoryTypeModel.find(req.body);
      return res.status(200).json(category);
    } catch (error) {
      res.status(500).json({ message: "ERROR", error: error.message });
    }
  ...
  async readByName(req, res) {
    try {
      ...
      const categoryType = await CategoryTypeModel.find({
        name: regexName,
      }).sort("name");
      return res.status(200).json(categoryType);
    } catch (error) {
      return res.status(500).json({
        message: "Erro while fetching CategoryType by name",
        error: error.message,
      });
    }
  }
  async readNames(req, res) {
    try {
      const names = await CategoryTypeModel.find({}, { name: 1 });
      const namesArray = names.map((ia) => ia.name);
      return res.status(200).json(namesArray);
    } catch (error) {
      res.status(500).json({
        message: "Error while fetching CategoryType names",
        error: error.message,
      });
    }
  }
  ...
  async destroy(req, res) {
    try {
      ...
      const foundCategory = await CategoryTypeModel.findById(id);
      if (!foundCategory) {
